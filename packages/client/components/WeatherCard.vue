    ...
    v-card-text
      v-row
        v-col(cols=12, sm=6)
          ...
          p.text-subtitle-1.mb-0 {{ Math.round(city.humidity) }}%
        v-col(cols=12, sm=6)
          p.text-body-1.text-uppercase.font-weight-bold.mb-0 pressure
          p.text-subtitle-1.mb-0 {{ Math.round(city.pressure) }} hPA
        v-col(cols=12, sm=6)
          p.text-body-1.text-uppercase.font-weight-bold.mb-0 Wind Speed
          p.text-subtitle-1.mb-0 {{ Math.round(city.wind.speed) }} m/s
        v-col(cols=12, sm=6)
          p.text-body-1.text-uppercase.font-weight-bold.mb-0 Feels Like
          p.text-subtitle-1.mb-0 {{ Math.round(city.temperatures.feel) }} &#8451;
        v-col(cols=12, sm=6)
          p.text-body-1.text-uppercase.font-weight-bold.mb-0 Visibility
          p.text-subtitle-1.mb-0 {{ Math.round(city.visibility) }} m
        v-col(cols=12, sm=6)
          p.text-body-1.text-uppercase.font-weight-bold.mb-0 Chance of Rain
          p.text-subtitle-1.mb-0 {{ Math.round(city.rainProbability) }}%
        v-col(cols=12, sm=6)
